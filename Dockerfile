...
#
ARG BUILDER_BASE=quay.io/khulnasoft/khulnasoft-envoy-builder:6.1.0-latest
...
WORKDIR /go/src/github.com/khulnasoft/proxy
COPY --chown=1337:1337 . ./
ARG TARGETARCH
ENV TARGETARCH=$TARGETARCH
RUN --mount=mode=0777,gid=1337,uid=1337,target=/khulnasoft/proxy/.cache,type=cache \
    --mount=mode=0777,gid=1337,uid=1337,target=/go/pkg,type=cache \
    PATH=$PATH:/usr/local/go/bin GOARCH=${TARGETARCH} make -C proxylib all && mv proxylib/libkhulnasoft.so /tmp/libkhulnasoft.so

FROM --platform=$BUILDPLATFORM $BUILDER_BASE AS builder-fresh
LABEL maintainer="maintainer@khulnasoft.io"
WORKDIR /khulnasoft/proxy
COPY . ./
ARG V
...
#
RUN --mount=mode=0777,uid=1337,gid=1337,target=/khulnasoft/proxy/.cache,type=cache,id=$TARGETARCH,sharing=private rm -rf /khulnasoft/proxy/.cache/*
#
# Build dependencies from scratch (no cache mounts, not archive mount)
#
RUN BAZEL_BUILD_OPTS="${BAZEL_BUILD_OPTS} --disk_cache=/tmp/bazel-cache" PKG_BUILD=1 V=$V DEBUG=$DEBUG DESTDIR=/tmp/install make bazel-bin/khulnasoft-envoy-starter bazel-bin/khulnasoft-envoy
...
RUN --mount=mode=0777,uid=1337,gid=1337,target=/khulnasoft/proxy/.cache,type=cache,id=$TARGETARCH,sharing=private \
    ...
    if [ -n "${COPY_CACHE_EXT}" ]; then PKG_BUILD=1 make BUILD_DEP_HASHES; if [ -f /tmp/bazel-cache/BUILD_DEP_HASHES ] && ! diff BUILD_DEP_HASHES /tmp/bazel-cache/BUILD_DEP_HASHES; then echo "Build dependencies have changed, clearing bazel cache"; rm -rf /tmp/bazel-cache/*; rm -rf /khulnasoft/proxy/.cache/*; fi ; cp BUILD_DEP_HASHES ENVOY_VERSION /tmp/bazel-cache; fi && \
...
#
COPY --from=proxylib /tmp/libkhulnasoft.so /tmp/install/usr/lib/libkhulnasoft.so
...
COPY --from=check-format /khulnasoft/proxy/format-output.txt /

#
# Extract installed khulnasoft-envoy binaries to an otherwise empty image
#
