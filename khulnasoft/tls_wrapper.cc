#include "khulnasoft/tls_wrapper.h"
...
// TLS configuration derived from Khulnasoft Network Policy.
...
public:
  ...
  void onConnected() override {
    // Get the Khulnasoft policy filter state from the callbacks in order to get the TLS
    // configuration.
    // Khulnasoft socket option is only created if the (initial) policy for the local pod exists.
    ...
    if (policy_socket_option) {
      ...
      if (!policy_socket_option->ingress_) {
        ...
        if (dst_address) {
          ...
          if (dip) {
            ...
            ENVOY_CONN_LOG(warn, "khulnasoft.tls_wrapper: Non-IP destination address: {}", conn,
                           dst_address->asString());
          }
        } else {
          ENVOY_CONN_LOG(warn, "khulnasoft.tls_wrapper: No destination address", conn);
        }
      }
      ...
      if (ctx) {
        ...
        ENVOY_CONN_LOG(
            warn,
            "khulnasoft.tls_wrapper: Could not get {} TLS context for pod {} on {} IP {} (id {}) port "
      ...
      ENVOY_CONN_LOG(warn,
                     "khulnasoft.tls_wrapper: Can not correlate connection with Khulnasoft Network "
                     "Policy (Khulnasoft socket option not found)",
                     conn);
    }
...
ProtobufTypes::MessagePtr UpstreamTlsWrapperFactory::createEmptyConfigProto() {
  return std::make_unique<::khulnasoft::UpstreamTlsWrapperContext>();
}
...
ProtobufTypes::MessagePtr DownstreamTlsWrapperFactory::createEmptyConfigProto() {
  return std::make_unique<::khulnasoft::DownstreamTlsWrapperContext>();
}
