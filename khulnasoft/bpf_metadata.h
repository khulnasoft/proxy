...
#include "khulnasoft/socket_option_source_address.h"
...
struct SocketMetadata : public Logger::Loggable<Logger::Id::filter> {
  ...
  std::shared_ptr<Envoy::Khulnasoft::KhulnasoftPolicyFilterState> buildKhulnasoftPolicyFilterState() {
    return std::make_shared<Envoy::Khulnasoft::KhulnasoftPolicyFilterState>(
  ...
  std::shared_ptr<Envoy::Khulnasoft::KhulnasoftMarkSocketOption> buildKhulnasoftMarkSocketOption() {
    return std::make_shared<Envoy::Khulnasoft::KhulnasoftMarkSocketOption>(mark_);
  };

  std::shared_ptr<Envoy::Khulnasoft::SourceAddressSocketOption> buildSourceAddressSocketOption() {
    return std::make_shared<Envoy::Khulnasoft::SourceAddressSocketOption>(
  ...
  void configureProxyLibApplicationProtocol(Network::ConnectionSocket& socket) {
    if (!proxylib_l7_proto_.empty()) {
      ...
      ENVOY_LOG(info, "khulnasoft.bpf_metadata: setRequestedApplicationProtocols(..., {})",
                proxylib_l7_proto_);
    }
  }
