#include "khulnasoft/websocket.h"
...
#include "khulnasoft/api/websocket.pb.h"
#include "khulnasoft/api/websocket.pb.validate.h" // IWYU pragma: keep
 ...
 */
class KhulnasoftWebSocketServerConfigFactory
...
public:
  ...
  createFilterFactoryFromProto(const Protobuf::Message& proto_config,
    ...
    auto config = std::make_shared<Khulnasoft::WebSocket::Config>(
        MessageUtil::downcastAndValidate<const ::khulnasoft::WebSocketServer&>(
            proto_config, context.messageValidationVisitor()),
        context);
    return [config](Network::FilterManager& filter_manager) mutable -> void {
      filter_manager.addFilter(std::make_shared<Khulnasoft::WebSocket::Instance>(config));
    };
  }

  ProtobufTypes::MessagePtr createEmptyConfigProto() override {
    return std::make_unique<::khulnasoft::WebSocketServer>();
  }

  std::string name() const override { return "khulnasoft.network.websocket.server"; }
};

/**
 * Static registration for the websocket server network filter. @see RegisterFactory.
 */
REGISTER_FACTORY(KhulnasoftWebSocketServerConfigFactory,
 ...
 * NamedNetworkFilterConfigFactory.
 */
class KhulnasoftWebSocketClientConfigFactory
    : public Server::Configuration::NamedNetworkFilterConfigFactory {
public:
  // NamedNetworkFilterConfigFactory
  absl::StatusOr<Network::FilterFactoryCb>
  createFilterFactoryFromProto(const Protobuf::Message& proto_config,
                               Server::Configuration::FactoryContext& context) override {
    auto config = std::make_shared<Khulnasoft::WebSocket::Config>(
        MessageUtil::downcastAndValidate<const ::khulnasoft::WebSocketClient&>(
            proto_config, context.messageValidationVisitor()),
        context);
    return [config](Network::FilterManager& filter_manager) mutable -> void {
      filter_manager.addFilter(std::make_shared<Khulnasoft::WebSocket::Instance>(config));
  ...
  ProtobufTypes::MessagePtr createEmptyConfigProto() override {
    return std::make_unique<::khulnasoft::WebSocketClient>();
  }

  std::string name() const override { return "khulnasoft.network.websocket.client"; }
};

/**
 * Static registration for the websocket client network filter. @see RegisterFactory.
 */
REGISTER_FACTORY(KhulnasoftWebSocketClientConfigFactory,
                 Server::Configuration::NamedNetworkFilterConfigFactory);
...
Network::FilterStatus Instance::onNewConnection() {
  ...
  ENVOY_CONN_LOG(debug, "khulnasoft.network.websocket: onNewConnection", conn);
...
Network::FilterStatus Instance::onWrite(Buffer::Instance& data, bool end_stream) {
  auto& conn = callbacks_->connection();
  ENVOY_CONN_LOG(trace, "khulnasoft.network.websocket: onWrite {} bytes, end_stream: {}", conn,
