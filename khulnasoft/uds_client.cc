...
UDSClient::UDSClient(const std::string& path, TimeSource& time_source)
  ...
  if (path.length() == 0) {
    throw EnvoyException(fmt::format("khulnasoft: Invalid Unix domain socket path: {}", path));
  }
...
void UDSClient::Log(const std::string& msg) {
  {
    ...
    while (tries-- > 0) {
      ...
      if (sent < length) {
        ENVOY_LOG(debug, "Khulnasoft access log send truncated by {} bytes.", length - sent);
      }
      return;
    }
  }
...
bool UDSClient::try_connect() {
  if (fd_ != -1) {
    ...
    ENVOY_LOG(debug, "Khulnasoft access log resetting socket due to error: {}",
