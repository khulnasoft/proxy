...
// by the Khulnasoft network- and HTTP policy filters via filter state.
class KhulnasoftPolicyFilterState : public StreamInfo::FilterState::Object,
                                public Logger::Loggable<Logger::Id::filter> {
public:
  KhulnasoftPolicyFilterState(uint32_t ingress_source_identity, uint32_t source_identity, bool ingress,
    ...
    ENVOY_LOG(
        debug,
        "Khulnasoft KhulnasoftPolicyFilterState(): source_identity: {}, "
  ...
  uint32_t resolvePolicyId(const Network::Address::Ip* ip) const {
    ...
    return Khulnasoft::ID::WORLD; // default to WORLD policy ID if resolver is no longer available
  }
