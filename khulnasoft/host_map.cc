...
protected:
  ...
  void insert(const khulnasoft::NetworkPolicyHosts& proto) {
...
PolicyHostMap::PolicyHostMap(ThreadLocal::SlotAllocator& tls) : tls_(tls.allocateSlot()) {
  instance_id_++;
  name_ = "khulnasoft.hostmap." + fmt::format("{}", instance_id_) + ".";
...
void PolicyHostMap::startSubscription(Server::Configuration::CommonFactoryContext& context) {
  subscription_ = subscribe("type.googleapis.com/khulnasoft.NetworkPolicyHosts", context.localInfo(),
                            context.clusterManager(), context.mainThreadDispatcher(),
                            context.api().randomGenerator(), *scope_, *this,
                            std::make_shared<Khulnasoft::PolicyHostDecoder>());
  subscription_->start({});
}
...
PolicyHostMap::onConfigUpdate(const std::vector<Envoy::Config::DecodedResourceRef>& resources,
  ...
  for (const auto& resource : resources) {
    const auto& config = dynamic_cast<const khulnasoft::NetworkPolicyHosts&>(resource.get().resource());
    ENVOY_LOG(trace,
