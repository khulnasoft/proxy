...
#include "khulnasoft/api/bpf_metadata.pb.validate.h" // IWYU pragma: keep
...
#include "khulnasoft/policy_id.h"
#include "khulnasoft/socket_option_khulnasoft_mark.h"
#include "khulnasoft/socket_option_ip_transparent.h"
...
public:
  ...
  Network::ListenerFilterFactoryCb createListenerFilterFactoryFromProto(
    ...
    auto config = std::make_shared<Khulnasoft::BpfMetadata::Config>(
        MessageUtil::downcastAndValidate<const ::khulnasoft::BpfMetadata&>(
            proto_config, context.messageValidationVisitor()),
        context);

    // Set the SO_MARK (Khulnasoft Mark), IP_TRANSPARENT & SO_REUSEADDR for the listen socket.
    ...
    options->push_back(std::make_shared<Khulnasoft::KhulnasoftMarkSocketOption>(mark));

    options->push_back(std::make_shared<Khulnasoft::IpTransparentSocketOption>());
    ...
    return [listener_filter_matcher,
      ...
      filter_manager.addAcceptFilter(listener_filter_matcher,
                                     std::make_unique<Khulnasoft::BpfMetadata::Instance>(config));
    };
  }

  ProtobufTypes::MessagePtr createEmptyConfigProto() override {
    return std::make_unique<::khulnasoft::BpfMetadata>();
  }

  std::string name() const override { return "khulnasoft.bpf_metadata"; }
};

/**
 * Static registration for the bpf metadata filter. @see RegisterFactory.
 * Versioning started from 1.1.0 for Khulnasoft version 1.12.0.
 */
  ...
  createFilterFactoryFromProto(const Protobuf::Message& proto_config,
    ...
    return [config](Network::UdpListenerFilterManager& udp_listener_filter_manager,
      ...
      udp_listener_filter_manager.addReadFilter(
          std::make_unique<Khulnasoft::BpfMetadata::UdpInstance>(config, callbacks));
    };
  }
...
SINGLETON_MANAGER_REGISTRATION(khulnasoft_bpf_conntrack);
SINGLETON_MANAGER_REGISTRATION(khulnasoft_host_map);
SINGLETON_MANAGER_REGISTRATION(khulnasoft_network_policy);

namespace {

std::shared_ptr<const Khulnasoft::PolicyHostMap>
createHostMap(Server::Configuration::ListenerFactoryContext& context) {
  return context.serverFactoryContext().singletonManager().getTyped<const Khulnasoft::PolicyHostMap>(
      SINGLETON_MANAGER_REGISTERED_NAME(khulnasoft_host_map), [&context] {
        auto map = std::make_shared<Khulnasoft::PolicyHostMap>(context.serverFactoryContext());
        map->startSubscription(context.serverFactoryContext());
        return map;
      });
}

std::shared_ptr<const Khulnasoft::NetworkPolicyMap>
createPolicyMap(Server::Configuration::FactoryContext& context, Khulnasoft::CtMapSharedPtr& ct) {
  return context.serverFactoryContext().singletonManager().getTyped<const Khulnasoft::NetworkPolicyMap>(
      SINGLETON_MANAGER_REGISTERED_NAME(khulnasoft_network_policy), [&context, &ct] {
        auto map = std::make_shared<Khulnasoft::NetworkPolicyMap>(context, ct);
...
Config::Config(const ::khulnasoft::BpfMetadata& config,
  ...
  if (is_l7lb_ && is_ingress_) {
    throw EnvoyException("khulnasoft.bpf_metadata: is_l7lb may not be set with is_ingress");
  }
  if ((ipv4_source_address_ &&
    ...
    throw EnvoyException(
        fmt::format("khulnasoft.bpf_metadata: ipv4_source_address is not an IPv4 address: {}",
  ...
  if ((ipv6_source_address_ &&
    ...
    throw EnvoyException(
        fmt::format("khulnasoft.bpf_metadata: ipv6_source_address is not an IPv6 address: {}",
  ...
  if (bpf_root.length() > 0) {
    ct_maps_ = context.serverFactoryContext().singletonManager().getTyped<Khulnasoft::CtMap>(
        SINGLETON_MANAGER_REGISTERED_NAME(khulnasoft_bpf_conntrack), [&bpf_root] {
          // Even if opening the global maps fail, local maps may still succeed
          // later.
          return std::make_shared<Khulnasoft::CtMap>(bpf_root);
        });
    ...
    if (bpf_root != ct_maps_->bpfRoot()) {
      ...
      throw EnvoyException(fmt::format("khulnasoft.bpf_metadata: Invalid bpf_root: {}", bpf_root));
    }
...
const PolicyInstance& Config::getPolicy(const std::string& pod_ip) const {
  ...
  // This is the case for L7 LB listeners only. This is needed to allow traffic forwarded by Khulnasoft
...
Config::extractSocketMetadata(Network::ConnectionSocket& socket) {
  ...
  if (is_l7lb_ && use_original_source_address_ /* East/West L7LB */) {
    ...
    if (policy->getEndpointID() == 0) {
      ...
      ENVOY_LOG(warn,
                "khulnasoft.bpf_metadata (east/west L7 LB): Non-local pod can not use original "
                "source address: {}",
                pod_ip);
      return absl::nullopt;
    }
    ...
    if (!ingress_ip) {
      ...
      ENVOY_LOG(
          warn,
          "khulnasoft.bpf_metadata (north/south L7 LB): No local Ingress IP source address configured "
          "for the family of {}",
          sip->addressAsString());
      return absl::nullopt;
    }
    ...
    if (source_identity == Khulnasoft::ID::WORLD) {
      // No security ID available for the configured source IP
      ENVOY_LOG(warn,
                "khulnasoft.bpf_metadata (north/south L7 LB): Unknown local Ingress IP source address "
                "configured: {}",
  ...
  if (policy->useProxylib(is_ingress_, remote_id, dip->port(), proxylib_l7proto)) {
    ENVOY_LOG(trace, "khulnasoft.bpf_metadata: detected proxylib l7 proto: {}", proxylib_l7proto);
  }
  ...
  ENVOY_LOG(trace,
            "khulnasoft.bpf_metadata: mark {}, ingress_source_identity {}, source_identity {}, "
  ...
  return absl::optional(Khulnasoft::BpfMetadata::SocketMetadata(
...
Network::FilterStatus Instance::onAccept(Network::ListenerFilterCallbacks& cb) {
  ...
  // Khulnasoft socket option is not set if this fails, which causes 500 response from our l7policy
  ...
  if (socket_metadata) {
    ...
    // Make Khulnasoft Policy data available to filters and upstream connection (Khulnasoft TLS Wrapper) as
    // filter state.
    cb.filterState().setData(
        Khulnasoft::KhulnasoftPolicyFilterState::key(), socket_metadata->buildKhulnasoftPolicyFilterState(),
    ...
    if (config_->addPrivilegedSocketOptions()) {
      // adding SO_MARK (Khulnasoft mark) on the upstream socket
      socket_options->push_back(socket_metadata->buildKhulnasoftMarkSocketOption());
    }
  }

  if (config_->addPrivilegedSocketOptions()) {
    // Setting IP_TRANSPARENT on upstream socket to be able to restore original source address
    socket_options->push_back(std::make_shared<Envoy::Khulnasoft::IpTransparentSocketOption>());
  }
