...
ProtocolClient::ProtocolClient()
    : Protocol(KHULNASOFT_PRIVILEGED_SERVICE_FD), call_mutex_(PTHREAD_MUTEX_INITIALIZER), seq_(0) {
  ...
  RELEASE_ASSERT((get_capabilities(CAP_EFFECTIVE) & ~(1UL << CAP_NET_BIND_SERVICE)) == 0 &&
                 ...
                 "khulnasoft-envoy running with privileges, exiting");

  if (!check_privileged_service()) {
    // No Khulnasoft privileged service detected
    close();
  }
...
ssize_t ProtocolClient::transact(MessageHeader& req, size_t req_len, const void* data,
  ...
  // Serialize calls to khulnasoft privileged service
  ...
  RELEASE_ASSERT(size != 0, "Khulnasoft privileged service closed pipe");
  RELEASE_ASSERT(size > 0, "Khulnasoft privileged service send failed");
  ...
  RELEASE_ASSERT(size < 0 || size_t(size) >= sizeof(resp),
                 "Khulnasoft privileged service truncated response");
  RELEASE_ASSERT(resp.hdr_.msg_seq_ == req.msg_seq_,
                 "Khulnasoft privileged service response out of sequence");
  RELEASE_ASSERT(resp.hdr_.msg_type_ == expected_response_type,
                 "Khulnasoft privileged service unexpected response type");
...
bool ProtocolClient::check_privileged_service() {
  ...
  ENVOY_LOG_MISC(debug, "Khulnasoft privileged service detected with following capabilities: {}", str);
  return true;
}

Envoy::Api::SysCallIntResult ProtocolClient::bpf_open(const char* path) {
  if (!have_khulnasoft_privileged_service()) {
    return {-1, EPERM};
  }
