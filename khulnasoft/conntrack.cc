...
CtMap::CtMaps4::CtMaps4(const std::string& bpf_root, const std::string& map_name) : ok_(false) {
  // Open the IPv4 bpf maps from Khulnasoft specific paths

  std::string path4tcp(bpf_root + "/tc/globals/khulnasoft_ct4_" + map_name);
  if (!ctmap4_tcp_.open(path4tcp)) {
    ENVOY_LOG(warn, "khulnasoft.bpf_metadata: Cannot open IPv4 conntrack map at {}", path4tcp);
    return;
  }
  std::string path4any(bpf_root + "/tc/globals/khulnasoft_ct_any4_" + map_name);
  if (!ctmap4_any_.open(path4any)) {
    ENVOY_LOG(info, "khulnasoft.bpf_metadata: Cannot open IPv4 conntrack map at {}", path4any);
...
CtMap::CtMaps6::CtMaps6(const std::string& bpf_root, const std::string& map_name) : ok_(false) {
  // Open the IPv6 bpf maps from Khulnasoft specific paths

  std::string path6tcp(bpf_root + "/tc/globals/khulnasoft_ct6_" + map_name);
  if (!ctmap6_tcp_.open(path6tcp)) {
    ENVOY_LOG(warn, "khulnasoft.bpf_metadata: Cannot open IPv6 conntrack map at {}", path6tcp);
    return;
  }
  std::string path6any(bpf_root + "/tc/globals/khulnasoft_ct_any6_" + map_name);
  if (!ctmap6_any_.open(path6any)) {
    ENVOY_LOG(info, "khulnasoft.bpf_metadata: Cannot open IPv6 conntrack map at {}", path6any);
    // do not fail if non-TCP map can not be opened
  }
...
CtMap::openMap4(const std::string& map_name) {
  ...
  ENVOY_LOG(debug, "khulnasoft.bpf_metadata: Opened IPv4 conntrack map {}", map_name);
  return pair.first;
}
...
CtMap::openMap6(const std::string& map_name) {
  ...
  ENVOY_LOG(debug, "khulnasoft.bpf_metadata: Opened IPv6 conntrack map {}", map_name);
  return pair.first;
}
...
void CtMap::closeMaps(const absl::flat_hash_set<std::string>& to_be_closed) {
  ...
  for (const auto& name : to_be_closed) {
    ...
    if (ct4 != ct_maps4_.end()) {
      ct_maps4_.erase(ct4);
      ENVOY_LOG(debug, "khulnasoft.bpf_metadata: Closed IPv4 conntrack map {}", name);
    }
    auto ct6 = ct_maps6_.find(name);
    if (ct6 != ct_maps6_.end()) {
      ct_maps6_.erase(ct6);
      ENVOY_LOG(debug, "khulnasoft.bpf_metadata: Closed IPv6 conntrack map {}", name);
    }
  }
}
...
CtMap::CtMap(const std::string& bpf_root) : bpf_root_(bpf_root) {
  if (openMap4("global") == ct_maps4_.end() && openMap6("global") == ct_maps6_.end()) {
    ENVOY_LOG(debug, "khulnasoft.bpf_metadata: conntrack map global open failed: ({})",
...
uint32_t CtMap::lookupSrcIdentity(const std::string& map_name, const Network::Address::Ip* sip,
  ...
  ENVOY_LOG(debug, "khulnasoft.bpf_metadata: Using conntrack map {}", map_name);
  ...
  if (sip->version() == Network::Address::IpVersion::v4 &&
    ...
    ENVOY_LOG(trace,
              "khulnasoft.bpf_metadata: Looking up key: {:x}, {:x}, {:x}, {:x}, "
              "{:x}, {:x}",
    ...
    ENVOY_LOG(info, "khulnasoft.bpf_metadata: Address type mismatch: Source: {}, Dest: {}",
  ...
  if (dip->version() == Network::Address::IpVersion::v4) {
    ...
    if (it == ct_maps4_.end()) {
      ENVOY_LOG(error, "khulnasoft.bpf_metadata: No IPv4 conntrack map {}", map_name);
      return 0;
    }
    ...
    if (!ct->ctmap4_tcp_.lookup(&key4, &value)) {
      ...
      ENVOY_LOG(debug, "khulnasoft.bpf_metadata: IPv4 conntrack map {} lookup failed: {}", map_name,
                Envoy::errorDetails(errno));
      return 0;
    }
  } else {
    ...
    if (it == ct_maps6_.end()) {
      ENVOY_LOG(error, "khulnasoft.bpf_metadata: No IPv6 conntrack map {}", map_name);
      return 0;
    }
    auto ct = it->second.get();
    if (!ct->ctmap6_tcp_.lookup(&key6, &value)) {
      ct_maps6_.erase(it); // flush the map to force reload after each failure.
      ENVOY_LOG(debug, "khulnasoft.bpf_metadata: IPv6 conntrack map {} lookup failed: {}", map_name,
                Envoy::errorDetails(errno));
      return 0;
    }
  }
