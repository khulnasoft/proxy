...
void AccessLog::Log(AccessLog::Entry& entry__, ::khulnasoft::EntryType entry_type) {
  ::khulnasoft::LogEntry& entry = entry__.entry_;
  entry.set_entry_type(entry_type);

  if (entry_type != ::khulnasoft::EntryType::Response) {
    if (entry__.request_logged_) {
      ENVOY_LOG_MISC(warn, "khulnasoft.AccessLog: Request is logged twice");
    }
...
void AccessLog::Entry::InitFromRequest(const std::string& policy_name, uint32_t proxy_id,
  ...
  ::khulnasoft::HttpProtocol proto;
  switch (info.protocol() ? info.protocol().value() : Http::Protocol::Http11) {
  case Http::Protocol::Http10:
    proto = ::khulnasoft::HttpProtocol::HTTP10;
    break;
  case Http::Protocol::Http11:
  default: // Just to make compiler happy
    proto = ::khulnasoft::HttpProtocol::HTTP11;
    break;
  case Http::Protocol::Http2:
    proto = ::khulnasoft::HttpProtocol::HTTP2;
    break;
  }
  ::khulnasoft::HttpLogEntry* http_entry = entry_.mutable_http();
...
void AccessLog::Entry::UpdateFromRequest(
  ...
  headers.iterate([http_entry](const Http::HeaderEntry& header) -> Http::HeaderMap::Iterate {
    ...
    if (key == pathSV) {
      ...
      ::khulnasoft::KeyValue* kv = http_entry->add_headers();
...
void AccessLog::Entry::AddRejected(absl::string_view key, absl::string_view value) {
  for (auto entry : entry_.http().rejected_headers())
    if (entry.key() == key && entry.value() == value)
      return;
  ::khulnasoft::KeyValue* kv = entry_.mutable_http()->add_rejected_headers();
  kv->set_key(key.data(), key.size());
  kv->set_value(value.data(), value.size());
}
...
void AccessLog::Entry::AddMissing(absl::string_view key, absl::string_view value) {
  for (auto entry : entry_.http().missing_headers())
    if (entry.key() == key && entry.value() == value)
      return;
  ::khulnasoft::KeyValue* kv = entry_.mutable_http()->add_missing_headers();
  kv->set_key(key.data(), key.size());
  kv->set_value(value.data(), value.size());
}
