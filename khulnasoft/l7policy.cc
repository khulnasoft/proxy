...
#include "khulnasoft/api/l7policy.pb.validate.h" // IWYU pragma: keep
#include "khulnasoft/filter_state_khulnasoft_policy.h"

namespace Envoy {
namespace Khulnasoft {

class KhulnasoftAccessFilterFactory
    : public Extensions::HttpFilters::Common::DualFactoryBase<::khulnasoft::L7Policy> {
public:
  KhulnasoftAccessFilterFactory() : DualFactoryBase("khulnasoft.l7policy") {}

private:
  absl::StatusOr<Http::FilterFactoryCb>
  createFilterFactoryFromProtoTyped(const ::khulnasoft::L7Policy& proto_config, const std::string&,
                                    DualInfo dual_info,
                                    Server::Configuration::ServerFactoryContext& context) override {
    auto config = std::make_shared<Khulnasoft::Config>(proto_config, context.timeSource(),
                                                   dual_info.scope, dual_info.is_upstream);
    return [config](Http::FilterChainFactoryCallbacks& callbacks) mutable -> void {
      callbacks.addStreamFilter(std::make_shared<Khulnasoft::AccessFilter>(config));
    };
  }
};

using UpstreamKhulnasoftAccessFilterFactory = KhulnasoftAccessFilterFactory;

/**
 * Static registration for this filter. @see RegisterFactory.
 */
REGISTER_FACTORY(KhulnasoftAccessFilterFactory, Server::Configuration::NamedHttpFilterConfigFactory);
REGISTER_FACTORY(UpstreamKhulnasoftAccessFilterFactory,
...
Config::Config(const std::string& access_log_path, const std::string& denied_403_body,
    ...
    : time_source_(time_source), stats_{ALL_KHULNASOFT_STATS(POOL_COUNTER_PREFIX(scope, "khulnasoft"))},
...
Config::Config(const ::khulnasoft::L7Policy& config, TimeSource& time_source, Stats::Scope& scope,
               bool is_upstream)
    : Config(config.access_log_path(), config.denied_403_body(), time_source, scope, is_upstream) {}

void Config::Log(AccessLog::Entry& entry, ::khulnasoft::EntryType type) {
  if (access_log_) {
...
void AccessFilter::onUpstreamConnectionEstablished() {
  if (latched_end_stream_.has_value()) {
    ...
    ENVOY_CONN_LOG(debug,
                   "khulnasoft.l7policy: RESUMING after upstream connection has been established",
...
Http::FilterHeadersStatus AccessFilter::decodeHeaders(Http::RequestHeaderMap& headers,
  ...
  if (config_->is_upstream_) {
    ...
    if (!callbacks_->upstreamCallbacks()->upstream()) {
      ...
      ENVOY_LOG(debug, "khulnasoft.l7policy: PAUSING until upstream connection has been established");
  ...
  if (!conn) {
    sendLocalError("khulnasoft.l7policy: No connection");
    return Http::FilterHeadersStatus::StopIteration;
  }

  ENVOY_CONN_LOG(debug, "khulnasoft.l7policy: {} decodeHeaders()", conn.ref(),
  ...
  if (!policy_fs) {
    sendLocalError("khulnasoft.l7policy: Khulnasoft policy filter state not found");
    return Http::FilterHeadersStatus::StopIteration;
  }
  ...
  if (nullptr == dst_address) {
    sendLocalError("khulnasoft.l7policy: No destination address");
    return Http::FilterHeadersStatus::StopIteration;
  }
  ...
  if (!dip) {
    sendLocalError(
        fmt::format("khulnasoft.l7policy: Non-IP destination address: {}", dst_address->asString()));
    return Http::FilterHeadersStatus::StopIteration;
  }

  if (log_entry_ == nullptr) {
    sendLocalError("khulnasoft.l7policy: No log entry");
    return Http::FilterHeadersStatus::StopIteration;
  }
  ...
  ENVOY_CONN_LOG(
      debug, "khulnasoft.l7policy: {} ({}->{}) {} policy lookup for endpoint {} for port {}: {}",
...
Http::FilterHeadersStatus AccessFilter::encodeHeaders(Http::ResponseHeaderMap& headers, bool) {
  ...
  ENVOY_CONN_LOG(debug, "khulnasoft.l7policy: {} encodeHeaders()", callbacks_->connection().ref(),
                 config_->is_upstream_ ? "upstream" : "downstream");
  ...
  if (!log_entry_->request_logged_) {
    ...
    auto logType = ::khulnasoft::EntryType::Request;

    if (headers.Status()->value() == "403") {
      // Log as a denied request.
      logType = ::khulnasoft::EntryType::Denied;
