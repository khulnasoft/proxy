...
bool Bpf::open(const std::string& path) {
  ...
  auto ret = khulnasoft_calls.bpf_open(path.c_str());
  ...
  if (fd_ >= 0) {
    ...
    if (bpf_file.is_open()) {
      ...
      if (log_on_error) {
        if (map_type != map_type_) {
          ENVOY_LOG(warn, "khulnasoft.bpf_metadata: map type mismatch on {}: got {}, wanted {}", path,
                    map_type, map_type_);
        } else if (key_size != key_size_) {
          ENVOY_LOG(warn,
                    "khulnasoft.bpf_metadata: map key size mismatch on {}: got {}, "
                    "wanted {}",
                    path, key_size, key_size_);
        } else {
          ENVOY_LOG(warn,
                    "khulnasoft.bpf_metadata: map value size mismatch on {}: got "
                    "{}, wanted {}-{}",
                    path, value_size, min_value_size_, max_value_size_);
        }
      }
    } else if (log_on_error) {
      ENVOY_LOG(warn, "khulnasoft.bpf_metadata: map {} could not open bpf file {}", path,
                bpf_file_path);
    }
    close();
  } else if (ret.errno_ == ENOENT && log_on_error) {
    ENVOY_LOG(debug, "khulnasoft.bpf_metadata: bpf syscall for map {} failed: {}", path,
              Envoy::errorDetails(ret.errno_));
  } else if (log_on_error) {
    ENVOY_LOG(warn, "khulnasoft.bpf_metadata: bpf syscall for map {} failed: {}", path,
              Envoy::errorDetails(ret.errno_));
  }
...
bool Bpf::lookup(const void* key, void* value) {
  ...
  auto& khulnasoft_calls = PrivilegedService::Singleton::get();
  auto result = khulnasoft_calls.bpf_lookup(fd_, key, key_size_, value, real_value_size_);
