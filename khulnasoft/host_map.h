...
#include "khulnasoft/api/nphds.pb.h"
#include "khulnasoft/api/nphds.pb.validate.h" // IWYU pragma: keep
...
public:
  ...
  ProtobufTypes::MessagePtr decodeResource(const ProtobufWkt::Any& resource) override {
    auto typed_message = std::make_unique<khulnasoft::NetworkPolicyHosts>();
    ...
    if (!resource.type_url().empty()) {
      MessageUtil::anyConvertAndValidate<khulnasoft::NetworkPolicyHosts>(resource, *typed_message,
                                                                     validation_visitor_);
    }
  ...
  std::string resourceName(const Protobuf::Message& resource) override {
    return fmt::format("{}", dynamic_cast<const khulnasoft::NetworkPolicyHosts&>(resource).policy());
  }
...
public:
  ...
  ~PolicyHostMap() {
    ENVOY_LOG(debug, "Khulnasoft PolicyHostMap({}): PolicyHostMap is deleted NOW!", name_);
  }
