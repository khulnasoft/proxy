...
TypeUrlToServiceMap* buildTypeUrlToServiceMap() {
  ...
  for (absl::string_view name : {
           "khulnasoft.NetworkPolicyDiscoveryService",
           "khulnasoft.NetworkPolicyHostsDiscoveryService",
       }) {
...
// Hard-coded Khulnasoft gRPC cluster
// Note: No rate-limit settings are used, consider if needed.
envoy::config::core::v3::ConfigSource getKhulnasoftXDSAPIConfig() {
  ...
  api_config_source->add_grpc_services()->mutable_envoy_grpc()->set_cluster_name("xds-grpc-khulnasoft");
  return config_source;
}

envoy::config::core::v3::ConfigSource khulnasoft_xds_api_config = getKhulnasoftXDSAPIConfig();
...
subscribe(const std::string& type_url, const LocalInfo::LocalInfo& local_info,
  ...
  const envoy::config::core::v3::ApiConfigSource& api_config_source =
      khulnasoft_xds_api_config.api_config_source();
