...
GoFilter::GoFilter(const std::string& go_module,
  ...
  if (!go_module_handle_) {
    throw EnvoyException(
        fmt::format("khulnasoft.network: Cannot load go module \'{}\': {}", go_module, dlerror()));
  }

  go_close_module_ = GoCloseModuleCB(::dlsym(go_module_handle_, "CloseModule"));
  if (!go_close_module_) {
    throw EnvoyException(fmt::format("khulnasoft.network: Cannot find symbol \'CloseModule\' from "
                                     "module \'{}\': {}",
                                     go_module, dlerror()));
  }
  GoOpenModuleCB go_open_module = GoOpenModuleCB(::dlsym(go_module_handle_, "OpenModule"));
  if (!go_open_module) {
    throw EnvoyException(fmt::format("khulnasoft.network: Cannot find symbol \'OpenModule\' from "
                                     "module \'{}\': {}",
                                     go_module, dlerror()));
  } else {
    ...
    if (go_module_id_ == 0) {
      throw EnvoyException(
          fmt::format("khulnasoft.network: \'{}::OpenModule()\' rejected parameters", go_module));
    }
  }

  go_on_new_connection_ = GoOnNewConnectionCB(::dlsym(go_module_handle_, "OnNewConnection"));
  if (!go_on_new_connection_) {
    throw EnvoyException(fmt::format("khulnasoft.network: Cannot find symbol \'OnNewConnection\' "
                                     "from module \'{}\': {}",
                                     go_module, dlerror()));
  }
  ...
  if (!go_on_data_) {
    throw EnvoyException(
        fmt::format("khulnasoft.network: Cannot find symbol \'OnData\' from module \'{}\': {}",
                    go_module, dlerror()));
  }
  go_close_ = GoCloseCB(::dlsym(go_module_handle_, "Close"));
  if (!go_close_) {
    throw EnvoyException(
        fmt::format("khulnasoft.network: Cannot find symbol \'Close\' from module \'{}\': {}",
                    go_module, dlerror()));
  }
}
...
GoFilter::InstancePtr GoFilter::NewInstance(Network::Connection& conn, const std::string& go_proto,
  ...
  if (go_module_handle_) {
    ...
    if (res == FILTER_OK) {
      ...
      ENVOY_CONN_LOG(warn, "Khulnasoft Network: Connection with parser \"{}\" rejected: {}", conn,
...
FilterResult GoFilter::Instance::OnIO(bool reply, Buffer::Instance& data, bool end_stream) {
  ...
  if (dir.pass_bytes_ > 0) {
    ...
    if (dir.pass_bytes_ > input_len) {
      if (input_len > 0) {
        ENVOY_CONN_LOG(debug, "Khulnasoft Network::OnIO: Passing all input: {} bytes: {} ", conn_,
    ...
    if (dir.drop_bytes_ > 0) {
      ...
      if (dir.drop_bytes_ > input_len) {
        if (input_len > 0) {
          ENVOY_CONN_LOG(debug, "Khulnasoft Network::OnIO: Dropping all input: {} bytes: {} ", conn_,
                         input_len, data.toString());
      ...
      ENVOY_CONN_LOG(debug, "Khulnasoft Network::OnIO: Dropping first {} bytes of input: {}", conn_,
  ...
  // above.
  if (dir.pass_bytes_ > 0) {
    ENVOY_CONN_LOG(debug, "Khulnasoft Network::OnIO: Passing first {} bytes of input: {}", conn_,
  ...
  if (dir.inject_slice_.len() > 0) {
    ENVOY_CONN_LOG(
        debug, "Khulnasoft Network::OnIO: Reverse Injecting: {} bytes: {} ", conn_,
  ...
  do {
    ...
    ENVOY_CONN_LOG(trace, "Khulnasoft Network::OnIO: Calling go module with {} bytes of data", conn_,
                   total_length);
    res = (*parent_.go_on_data_)(connection_id_, reply, end_stream, &input_slices, &ops);
    ENVOY_CONN_LOG(trace, "Khulnasoft Network::OnIO: \'go_on_data\' returned {}, ops({})", conn_,
    ...
    if (res == FILTER_OK) {
      ...
      for (int i = 0; i < ops.len(); i++) {
        ...
        if (n_bytes == 0) {
          ENVOY_CONN_LOG(warn, "Khulnasoft Network::OnIO: INVALID op ({}) length: {} bytes", conn_, op,
                         n_bytes);
        ...
        if (terminal_op_seen) {
          ENVOY_CONN_LOG(warn,
                         "Khulnasoft Network::OnIO: Filter operation {} after "
                         "terminal operation.",
                         conn_, op);
          return FILTER_PARSER_ERROR;
        }
        ...
        switch (op) {
        case FILTEROP_MORE:
          ENVOY_CONN_LOG(debug, "Khulnasoft Network::OnIO: FILTEROP_MORE: {} bytes", conn_, n_bytes);
        ...
        case FILTEROP_PASS:
          ENVOY_CONN_LOG(debug, "Khulnasoft Network::OnIO: FILTEROP_PASS: {} bytes", conn_, n_bytes);
        ...
        case FILTEROP_DROP:
          ENVOY_CONN_LOG(debug, "Khulnasoft Network::OnIO: FILTEROP_DROP: {} bytes", conn_, n_bytes);
          if (n_bytes > input_len) {
        ...
        case FILTEROP_INJECT:
          if (n_bytes > dir.inject_slice_.len()) {
            ENVOY_CONN_LOG(warn,
                           "Khulnasoft Network::OnIO: FILTEROP_INJECT: INVALID "
                           "length: {} bytes",
                           conn_, n_bytes);
            return FILTER_PARSER_ERROR;
          }
          ENVOY_CONN_LOG(debug, "Khulnasoft Network::OnIO: FILTEROP_INJECT: {} bytes: {}", conn_,
                         n_bytes,
        ...
        case FILTEROP_ERROR:
        default:
          ENVOY_CONN_LOG(warn, "Khulnasoft Network::OnIO: FILTEROP_ERROR: {} bytes", conn_, n_bytes);
          return FILTER_PARSER_ERROR;
        }
      }
    } else {
      // Close the connection an any error
      ENVOY_CONN_LOG(warn, "Khulnasoft Network::OnIO: FILTER_POLICY_DROP {}", conn_, toString(res));
      return FILTER_PARSER_ERROR;
    }

    if (dir.inject_slice_.len() > 0) {
      ENVOY_CONN_LOG(warn, "Khulnasoft Network::OnIO: {} bytes abandoned in inject buffer", conn_,
                     dir.inject_slice_.len());
      return FILTER_PARSER_ERROR;
    }
  ...
  if (output.length() < 100) {
    ENVOY_CONN_LOG(debug, "Khulnasoft Network::OnIO: Output on return: {}", conn_, output.toString());
  } else {
    ENVOY_CONN_LOG(debug, "Khulnasoft Network::OnIO: Output length return: {}", conn_, output.length());
  }
  return res;
}
