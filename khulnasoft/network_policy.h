...
#include "khulnasoft/api/npds.pb.validate.h" // IWYU pragma: keep
...
// use of named ports and numbered ports in Khulnasoft Network Policy at the same time).
...
public:
  ...
  IPAddressPair(const khulnasoft::NetworkPolicy& proto);
...
public:
  ...
  virtual bool allowed(bool ingress, uint32_t remote_id, uint16_t port,
                       ...
                       Khulnasoft::AccessLog::Entry& log_entry) const PURE;
...
public:
  ...
  ProtobufTypes::MessagePtr decodeResource(const ProtobufWkt::Any& resource) override {
    auto typed_message = std::make_unique<khulnasoft::NetworkPolicy>();
    ...
    if (!resource.type_url().empty()) {
      MessageUtil::anyConvertAndValidate<khulnasoft::NetworkPolicy>(resource, *typed_message,
                                                                validation_visitor_);
    }
  ...
  std::string resourceName(const Protobuf::Message& resource) override {
    return fmt::format("{}", dynamic_cast<const khulnasoft::NetworkPolicy&>(resource).endpoint_id());
  }
 ...
 */
// clang-format off
#define ALL_KHULNASOFT_POLICY_STATS(COUNTER, HISTOGRAM)	\
 ...
 */
struct PolicyStats {
  ALL_KHULNASOFT_POLICY_STATS(GENERATE_COUNTER_STRUCT, GENERATE_HISTOGRAM_STRUCT)
};
...
public:
  ...
  NetworkPolicyMap(Server::Configuration::FactoryContext& context, Khulnasoft::CtMapSharedPtr& ct);
  ~NetworkPolicyMap();
...
private:
  ...
  friend class KhulnasoftNetworkPolicyTest;
  ...
  Khulnasoft::CtMapSharedPtr ctmap_;
