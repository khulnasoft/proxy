...
Config::Config(Server::Configuration::FactoryContext& context, bool client,
  ...
  if (interval > 0) {
    ...
    throw EnvoyException(
        "khulnasoft.network.websocket: ping_when_idle requires ping_interval to be set.");
  }
}

Config::Config(const ::khulnasoft::WebSocketClient& config,
  ...
  // Client defaults
  if (host_.empty()) {
    throw EnvoyException("khulnasoft.network.websocket.client: host must be non-empty.");
  }
...
Config::Config(const ::khulnasoft::WebSocketServer& config,
               Server::Configuration::FactoryContext& context)
