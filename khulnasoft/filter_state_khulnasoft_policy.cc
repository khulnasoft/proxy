...
const std::string& KhulnasoftPolicyFilterState::key() {
  CONSTRUCT_ON_FIRST_USE(std::string, "khulnasoft.policy");
}

bool KhulnasoftPolicyFilterState::enforceNetworkPolicy(const Network::Connection& conn,
...
bool KhulnasoftPolicyFilterState::enforceHTTPPolicy(const Network::Connection& conn, bool is_downstream,
                                                uint32_t destination_identity,
                                                uint16_t destination_port,
