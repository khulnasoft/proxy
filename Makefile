# Copyright 2017-2021 Authors of Khulnasoft
#
...
ENVOY_BINS = khulnasoft-envoy bazel-bin/khulnasoft-envoy khulnasoft-envoy-starter bazel-bin/khulnasoft-envoy-starter
...
ifdef PKG_BUILD
  ...
  all: khulnasoft-envoy-starter khulnasoft-envoy
  ...
  all: precheck khulnasoft-envoy-starter khulnasoft-envoy
...
.PHONY: bazel-bin/khulnasoft-envoy
bazel-bin/khulnasoft-envoy: $(COMPILER_DEP) SOURCE_VERSION install-bazelisk
	@$(ECHO_BAZEL)
	$(BAZEL) $(BAZEL_OPTS) build $(BAZEL_BUILD_OPTS) //:khulnasoft-envoy $(BAZEL_FILTER)

khulnasoft-envoy: bazel-bin/khulnasoft-envoy
	mv $< $@

.PHONY: bazel-bin/khulnasoft-envoy-starter
bazel-bin/khulnasoft-envoy-starter: $(COMPILER_DEP) SOURCE_VERSION install-bazelisk
	@$(ECHO_BAZEL)
	$(BAZEL) $(BAZEL_OPTS) build $(BAZEL_BUILD_OPTS) //:khulnasoft-envoy-starter $(BAZEL_FILTER)

khulnasoft-envoy-starter: bazel-bin/khulnasoft-envoy-starter
	mv $< $@
...
$(DESTDIR)$(GLIBC_DIR): bazel-bin/khulnasoft-envoy
	$(SUDO) $(INSTALL) -m 0755 -d $@
	LIBS=$$(readelf -d bazel-bin/khulnasoft-envoy | sed -n 's/.*(NEEDED).*Shared library: \[\(.*\)\]/\1/p'); \
...
install: bazel-bin/khulnasoft-envoy-starter bazel-bin/khulnasoft-envoy
	$(SUDO) $(INSTALL) -m 0755 -d $(DESTDIR)$(BINDIR)
	$(SUDO) $(INSTALL) -m 0755 -T bazel-bin/khulnasoft-envoy-starter $(DESTDIR)$(BINDIR)/khulnasoft-envoy-starter
	$(SUDO) $(INSTALL) -m 0755 -T bazel-bin/khulnasoft-envoy $(DESTDIR)$(BINDIR)/khulnasoft-envoy

install-glibc: install $(DESTDIR)$(GLIBC_DIR)
	LD_LINUX=$$(basename $$(patchelf --print-interpreter bazel-bin/khulnasoft-envoy)); \
	$(SUDO) patchelf --set-interpreter $(GLIBC_DIR)/$${LD_LINUX} --set-rpath $(GLIBC_DIR) $(DESTDIR)$(BINDIR)/khulnasoft-envoy-starter
	$(SUDO) patchelf --set-interpreter $(GLIBC_DIR)/$${LD_LINUX} --set-rpath $(GLIBC_DIR) $(DESTDIR)$(BINDIR)/khulnasoft-envoy
...
envoy-test-deps: $(COMPILER_DEP) SOURCE_VERSION proxylib/libkhulnasoft.so
	@$(ECHO_BAZEL)
...
.PHONY: envoy-tests
envoy-tests: $(COMPILER_DEP) SOURCE_VERSION proxylib/libkhulnasoft.so
	@$(ECHO_BAZEL)
