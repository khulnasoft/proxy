...
int main(int argc, char** argv) {
  ...
  uint64_t caps = Envoy::Khulnasoft::PrivilegedService::get_capabilities(CAP_EFFECTIVE);
  if ((caps & (1UL << CAP_NET_ADMIN)) == 0 ||
    ...
    fprintf(stderr, "CAP_NET_ADMIN and either CAP_SYS_ADMIN or CAP_BPF capabilities are needed for "
                    "Khulnasoft datapath integration.\n");
    exit(1);
  }
  ...
  if (pid == 0) {
    ...
    RELEASE_ASSERT(
        Envoy::Khulnasoft::PrivilegedService::get_capabilities(CAP_EFFECTIVE) == exp_eff_cap &&
            Envoy::Khulnasoft::PrivilegedService::get_capabilities(CAP_PERMITTED) == exp_perm_cap &&
            Envoy::Khulnasoft::PrivilegedService::get_capabilities(CAP_INHERITABLE) == 0,
        "Failed dropping privileges");

    // Dup the client end to KHULNASOFT_PRIVILEGED_SERVICE_FD
    if (fds[1] != KHULNASOFT_PRIVILEGED_SERVICE_FD) {
      if (dup2(fds[1], KHULNASOFT_PRIVILEGED_SERVICE_FD) < 0) {
        perror("dup2");
        exit(1);
      }
      close(fds[1]);
  ...
  Envoy::Khulnasoft::PrivilegedService::ProtocolServer server(pid, fds[0]);
  server.serve();
  return 0;
}
