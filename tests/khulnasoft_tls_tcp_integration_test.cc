...
#include "tests/khulnasoft_tls_integration.h"
...
//
// Khulnasoft filters with TCP proxy & Upstream TLS
//

// params: is_ingress ("true", "false")
const std::string khulnasoft_tls_tcp_proxy_config_fmt = R"EOF(
admin:
  address:
    socket_address:
      address: 127.0.0.1
...
static_resources:
  ...
  - name: tls-cluster
    ...
    connect_timeout:
      seconds: 1
    transport_socket:
      name: "khulnasoft.tls_wrapper"
      typed_config:
        "@type": type.googleapis.com/khulnasoft.UpstreamTlsWrapperContext
...
class KhulnasoftTLSIntegrationTest : public KhulnasoftTcpIntegrationTest {
public:
  KhulnasoftTLSIntegrationTest(const std::string& config) : KhulnasoftTcpIntegrationTest(config) {
#if 0
  ...
  void initialize() override {
    KhulnasoftTcpIntegrationTest::initialize();
  ...
  void createUpstreams() override {
    if (upstream_tls_) {
      ...
      KhulnasoftTcpIntegrationTest::createUpstreams(); // maybe BaseIntegrationTest::createUpstreams()
    }
  }
...
class KhulnasoftTLSProxyIntegrationTest : public KhulnasoftTLSIntegrationTest {
public:
  KhulnasoftTLSProxyIntegrationTest()
      : KhulnasoftTLSIntegrationTest(fmt::format(
            fmt::runtime(TestEnvironment::substitute(khulnasoft_tls_tcp_proxy_config_fmt, GetParam())),
            "true")) {}
...
INSTANTIATE_TEST_SUITE_P(IpVersions, KhulnasoftTLSProxyIntegrationTest,
                         testing::ValuesIn(TestEnvironment::getIpVersionsForTest()),
                         TestUtility::ipTestParamsToString);

// Test upstream writing before downstream does.
TEST_P(KhulnasoftTLSProxyIntegrationTest, KhulnasoftTLSProxyUpstreamWritesFirst) {
  initialize();
...
TEST_P(KhulnasoftTLSProxyIntegrationTest, KhulnasoftTLSProxyUpstreamDisconnect) {
  initialize();
  IntegrationTcpClientPtr tcp_client = makeTcpConnection(lookupPort("tcp_proxy"));
  ASSERT_TRUE(tcp_client->write("hello"));
...
TEST_P(KhulnasoftTLSProxyIntegrationTest, KhulnasoftTcpProxyDownstreamDisconnect) {
  initialize();
  IntegrationTcpClientPtr tcp_client = makeTcpConnection(lookupPort("tcp_proxy"));
  ASSERT_TRUE(tcp_client->write("hello"));
...
TEST_P(KhulnasoftTLSProxyIntegrationTest, KhulnasoftTLSProxyLargeWrite) {
...
TEST_P(KhulnasoftTLSProxyIntegrationTest, KhulnasoftTLSProxyDownstreamFlush) {
...
TEST_P(KhulnasoftTLSProxyIntegrationTest, KhulnasoftTLSProxyUpstreamFlush) {
  // Use a very large size to make sure it is larger than the kernel socket read
  // buffer.
  const uint32_t size = 50 * 1024 * 1024;
...
// flush active
TEST_P(KhulnasoftTLSProxyIntegrationTest, KhulnasoftTLSProxyUpstreamFlushEnvoyExit) {
  // Use a very large size to make sure it is larger than the kernel socket read
  // buffer.
  const uint32_t size = 50 * 1024 * 1024;
  config_helper_.setBufferLimits(size, size);
...
// params: is_ingress ("true", "false")
const std::string khulnasoft_tls_downstream_tcp_proxy_config_fmt = R"EOF(
admin:
  address:
    socket_address:
      address: 127.0.0.1
  ...
  listeners:
    ...
    - filter_chain_match:
      ...
      transport_socket:
        name: "khulnasoft.tls_wrapper"
        typed_config:
          "@type": type.googleapis.com/khulnasoft.DownstreamTlsWrapperContext
      filters:
      - name: khulnasoft.network
        typed_config:
          "@type": type.googleapis.com/khulnasoft.NetworkFilter
...
class KhulnasoftDownstreamTLSIntegrationTest : public KhulnasoftTLSIntegrationTest {
public:
  KhulnasoftDownstreamTLSIntegrationTest()
      : KhulnasoftTLSIntegrationTest(
            fmt::format(fmt::runtime(TestEnvironment::substitute(
                            khulnasoft_tls_downstream_tcp_proxy_config_fmt, GetParam())),
                        "true")) {}
...
INSTANTIATE_TEST_SUITE_P(IpVersions, KhulnasoftDownstreamTLSIntegrationTest,
                         testing::ValuesIn(TestEnvironment::getIpVersionsForTest()),
                         TestUtility::ipTestParamsToString);

TEST_P(KhulnasoftDownstreamTLSIntegrationTest, SendTlsToTlsListener) {
  setupConnections();
  sendAndReceiveTlsData("hello", "world");
}

TEST_P(KhulnasoftDownstreamTLSIntegrationTest, LargeBidirectionalTlsWrites) {
  setupConnections();
...
TEST_P(KhulnasoftDownstreamTLSIntegrationTest, DownstreamHalfClose) {
  setupConnections();
...
TEST_P(KhulnasoftDownstreamTLSIntegrationTest, UpstreamHalfClose) {
  setupConnections();
