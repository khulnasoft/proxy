...
#include "khulnasoft/websocket_protocol.h"
...
static const char EXPECTED_HANDSHAKE_FMT[] =
    "GET / HTTP/1.1" CRLF "host: jarno.khulnasoft.rocks" CRLF "upgrade: websocket" CRLF
    "connection: upgrade" CRLF "sec-websocket-key: c3VwZXItc2VjcmV0LWtleQ==" CRLF
    "sec-websocket-version: 13" CRLF "origin: jarno.khulnasoft.rocks" CRLF
...
TEST_P(KhulnasoftWebSocketIntegrationTest, KhulnasoftWebSocketHandshakeNonHTTPResponse) {
  initialize();
...
TEST_P(KhulnasoftWebSocketIntegrationTest, KhulnasoftWebSocketHandshakeInvalidResponse) {
  initialize();
  IntegrationTcpClientPtr tcp_client = makeTcpConnection(lookupPort("tcp_proxy"));
  ASSERT_TRUE(tcp_client->write("hello"));
  FakeRawConnectionPtr fake_upstream_connection;
...
// created.
TEST_P(KhulnasoftWebSocketIntegrationTest, KhulnasoftWebSocketHandshakeSuccess) {
  initialize();
  IntegrationTcpClientPtr tcp_client = makeTcpConnection(lookupPort("tcp_proxy"));
  ASSERT_TRUE(tcp_client->write("hello"));
  FakeRawConnectionPtr fake_upstream_connection;
...
TEST_P(KhulnasoftWebSocketIntegrationTest, KhulnasoftWebSocketHandshakeNoData) {
  initialize();
  IntegrationTcpClientPtr tcp_client = makeTcpConnection(lookupPort("tcp_proxy"));
  FakeRawConnectionPtr fake_upstream_connection;
  ASSERT_TRUE(fake_upstreams_[0]->waitForRawConnection(fake_upstream_connection));
