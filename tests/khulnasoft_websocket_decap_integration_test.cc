...
class KhulnasoftWebSocketIntegrationTest : public KhulnasoftHttpIntegrationTest {
public:
...
TEST_P(KhulnasoftWebSocketIntegrationTest, DeniedNonWebSocket) {
  initialize();
  Denied({{":method", "GET"}, {":path", "/"}, {":authority", "host"}});
}

TEST_P(KhulnasoftWebSocketIntegrationTest, AcceptedWebSocket) {
  initialize();
