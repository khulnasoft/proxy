...
// Test Khulnasoft::BpfMetadata::Config filter config
// (NOT Khulnasoft::BpfMetadata::TestConfig)
...
- "@type": type.googleapis.com/khulnasoft.NetworkPolicy
  ...
  void initialize(const ::khulnasoft::BpfMetadata& config, std::string extra_host_map_config = "",
    ...
    config_ = std::make_shared<Khulnasoft::BpfMetadata::Config>(config, context_);
  ...
  std::shared_ptr<Khulnasoft::BpfMetadata::Config> config_;
...
TEST_F(MetadataConfigTest, InvalidL7lbConfig) {
  ...
  EXPECT_THROW_WITH_MESSAGE(initialize(config), EnvoyException,
                            "khulnasoft.bpf_metadata: is_l7lb may not be set with is_ingress");
}
...
TEST_F(MetadataConfigTest, InvalidIngressIpv4Address) {
  ...
  EXPECT_THROW_WITH_MESSAGE(
      ...
      "khulnasoft.bpf_metadata: ipv4_source_address is not an IPv4 address: invalid");
}
...
TEST_F(MetadataConfigTest, InvalidIngressIpv6Address) {
  ...
  EXPECT_THROW_WITH_MESSAGE(
      initialize(config), EnvoyException,
      "khulnasoft.bpf_metadata: ipv6_source_address is not an IPv6 address: invalid");
}
...
TEST_F(MetadataConfigTest, NorthSouthL7LbMetadata) {
  ...
  auto policy_fs = socket_metadata->buildKhulnasoftPolicyFilterState();
  ...
  auto khulnasoft_mark_socket_option = socket_metadata->buildKhulnasoftMarkSocketOption();
  EXPECT_NE(nullptr, khulnasoft_mark_socket_option);

  // Check that Ingress security ID is used in the socket mark
  EXPECT_TRUE((khulnasoft_mark_socket_option->mark_ & 0xffff) == 0x0B00 &&
              (khulnasoft_mark_socket_option->mark_ >> 16) == 8);
}
...
TEST_F(MetadataConfigTest, NorthSouthL7LbPodAndIngressEnforcedMetadata) {
  ...
  std::string extra_host_map_config = R"EOF(- "@type": type.googleapis.com/khulnasoft.NetworkPolicyHosts
  policy: 9999
  host_addresses: [ "192.168.1.1" ]
)EOF";

  std::string extra_policy_config = R"EOF(- "@type": type.googleapis.com/khulnasoft.NetworkPolicy
  endpoint_ips:
  - '192.168.1.1'
  - 'face:192:168:1:1'
  endpoint_id: 3000
...
TEST_F(MetadataConfigTest, EastWestL7LbMetadata) {
  ...
  EXPECT_TRUE((khulnasoft_mark_socket_option->mark_ & 0xffff) == 0x0900 &&
              (khulnasoft_mark_socket_option->mark_ >> 16) == 2048);
}
