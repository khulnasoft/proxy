...
std::shared_ptr<const Khulnasoft::PolicyHostMap> hostmap{nullptr}; // Keep reference to singleton

Network::Address::InstanceConstSharedPtr original_dst_address;
std::shared_ptr<const Khulnasoft::NetworkPolicyMap> npmap{nullptr}; // Keep reference to singleton
...
createHostMap(const std::string& config, Server::Configuration::ListenerFactoryContext& context) {
  return context.serverFactoryContext().singletonManager().getTyped<const Khulnasoft::PolicyHostMap>(
      "khulnasoft_host_map_singleton", [&config, &context] {
        std::string path = TestEnvironment::writeStringToFileForTest("host_map.yaml", config);
        ENVOY_LOG_MISC(debug, "Loading Khulnasoft Host Map from file \'{}\' instead of using gRPC",
                       path);
        ...
        auto subscription = std::make_unique<Envoy::Config::FilesystemSubscriptionImpl>(
            ...
            std::make_shared<Khulnasoft::PolicyHostDecoder>(), stats,
...
createPolicyMap(const std::string& config,
  ...
  return context.serverFactoryContext().singletonManager().getTyped<const Khulnasoft::NetworkPolicyMap>(
      "khulnasoft_network_policy_singleton", [&config, &secret_configs, &context] {
        ...
        ENVOY_LOG_MISC(debug,
                       "Loading Khulnasoft Network Policy from file \'{}\' instead "
                       "of using gRPC",
                       policy_path);
        THROW_IF_NOT_OK(Envoy::Config::Utility::checkFilesystemSubscriptionBackingPath(
        ...
        Envoy::Config::SubscriptionStats stats =
            Envoy::Config::Utility::generateStats(context.scope());
        auto map = std::make_shared<Khulnasoft::NetworkPolicyMap>(context);
        auto subscription = std::make_unique<Envoy::Config::FilesystemSubscriptionImpl>(
            context.serverFactoryContext().mainThreadDispatcher(),
            Envoy::Config::makePathConfigSource(policy_path), *map,
            std::make_shared<Khulnasoft::NetworkPolicyDecoder>(), stats,
            ProtobufMessage::getNullValidationVisitor(), context.serverFactoryContext().api());
        map->startSubscription(std::move(subscription));
        return map;
      });
...
namespace {
::khulnasoft::BpfMetadata getTestConfig(const ::khulnasoft::TestBpfMetadata& config) {
  ::khulnasoft::BpfMetadata test_config;
...
TestConfig::TestConfig(const ::khulnasoft::TestBpfMetadata& config,
...
TestConfig::extractSocketMetadata(Network::ConnectionSocket& socket) {
  ...
  // "default")?
  socket.setDetectedTransportProtocol("khulnasoft:default");
...
public:
  ...
  Network::ListenerFilterFactoryCb createListenerFilterFactoryFromProto(
    ...
    auto config = std::make_shared<Khulnasoft::BpfMetadata::TestConfig>(
        MessageUtil::downcastAndValidate<const ::khulnasoft::TestBpfMetadata&>(
  ...
  ProtobufTypes::MessagePtr createEmptyConfigProto() override {
    return std::make_unique<::khulnasoft::TestBpfMetadata>();
  }
