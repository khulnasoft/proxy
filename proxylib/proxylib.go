...
import (
	...
	"github.com/khulnasoft/proxy/proxylib/libkhulnasoft"
)
...
func OnNewConnection(instanceId uint64, proto string, connectionId uint64, ingress bool, srcId, dstId uint32, srcAddr, dstAddr, policyName string, origBuf, replyBuf *[]byte) C.FilterResult {
	return C.FilterResult(libkhulnasoft.OnNewConnection(instanceId, proto, connectionId, ingress, srcId, dstId, srcAddr, dstAddr, policyName, origBuf, replyBuf))
}
...
func OnData(connectionId uint64, reply, endStream bool, data *[][]byte, filterOps *[][2]int64) C.FilterResult {
	return C.FilterResult(libkhulnasoft.OnData(connectionId, reply, endStream, data, filterOps))
}
...
func Close(connectionId uint64) {
	libkhulnasoft.Close(connectionId)
}
...
func OpenModule(params [][2]string, debug bool) uint64 {
	return libkhulnasoft.OpenModule(params, debug)
}

//export CloseModule
func CloseModule(id uint64) {
	libkhulnasoft.CloseModule(id)
}
