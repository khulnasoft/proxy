...
// $ khulnasoft policy import proxylib/testparsers/headerparser.policy
//
...
func L7HeaderRuleParser(rule *khulnasoft.PortNetworkPolicyRule) []L7NetworkPolicyRule {
...
func (p *HeaderParser) OnData(reply, endStream bool, data [][]byte) (OpType, int) {
	...
	if reply || p.connection.Matches(line) {
		p.connection.Log(khulnasoft.EntryType_Request,
