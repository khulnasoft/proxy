...
import (
	...
	"github.com/khulnasoft/proxy/proxylib/test"
	_ "github.com/khulnasoft/proxy/proxylib/testparsers"
)
...
func TestOpenModule(t *testing.T) {
	...
	mod5 := OpenModule([][2]string{{"access-log-path", logServer.Path}, {"node-id", "host~127.0.0.1~libkhulnasoft~localdomain"}}, debug)
	if mod5 == 0 {
		t.Errorf("OpenModule() with access log path %s failed", logServer.Path)
	} else {
...
func checkAccessLogs(t *testing.T, logServer *test.AccessLogServer, expPasses, expDrops int) {
	...
	for !done && nWaits < 100 {
		...
		case entryType := <-logServer.Logs:
			if entryType == khulnasoft.EntryType_Denied {
				drops++
			} else {
				passes++
			}
...
func TestTwoRulesOnSamePortMismatchingL7(t *testing.T) {
	...
	// This registation will remain after this test.
	proxylib.RegisterL7RuleParser("PortNetworkPolicyRule_HttpRules", func(*khulnasoft.PortNetworkPolicyRule) []proxylib.L7NetworkPolicyRule {
		return nil
	})
